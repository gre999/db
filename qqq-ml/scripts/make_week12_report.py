...
**日期對齊確認**:手工特徵 logistic 用的是第11週用CNN同一個序列資料集(`SEQ.load_tensor
("task_a")`)重新配適的版本,不是第8週原始9折(2018–2026)的檔案,兩者日期集合完全相同
(共 {len(common_a)} 天,2019-01-02–2026-09-22,8折)——程式碼另外用`.intersection()`
再次確認過,沒有缺漏。手工特徵 logistic 的 {hand_auc:.4f} 跟第8週報告的0.597看起來很像
是巧合:模型定義本身沒變,拿第8週原始9折檔案限制到同樣2019–2026共同日期重算,得到
0.5971522831265627,跟這裡的{hand_auc:.4f}到小數點後至少10位完全一致;第8週原本的
9折(含2018)合併數字是0.5965,四捨五入也是0.597,只是巧合地跟這裡幾乎重疊,不代表
沿用了舊的、範圍不同的計算。堆疊比較(下面第三節)用的也是同一個{len(common_a)}天共同
日期集合,同樣verified過。

CNN 對手工特徵 logistic 的測試段 AUC 差,8折測試段全部合併成一條日期序列(n={len(common_a)}):
...
——堆疊AUC顯著低於手工特徵單獨。解讀:CNN分數在增量上沒有資訊,但堆疊仍在驗證年小樣本
(n≈248–251)上用無正則化logistic多估一個係數,多估的係數本身就會增加樣本外的估計雜訊
(第3折CNN係數-5.49、標準誤3.21,接近完全分離,就是這種雜訊的例證)。因此堆疊AUC較低,
判讀為「多一個沒有資訊量的係數,在小樣本上增加雜訊」,不解讀為CNN分數本身主動有害。
...
(CNN是顯著更差,不是接近),也不是「CNN明顯勝過」。如實記錄:**CNN 沒有增量資訊
(Spearman中等、R²中等未達0.6門檻、堆疊係數逐折不同號且僅1/8折顯著);堆疊AUC顯著
低於手工特徵單獨,判讀為小樣本下多估一個無資訊量係數所增加的估計雜訊,不是CNN分數
本身對預測主動有害**。
...
**CNN 從原始分鐘序列學到的,是否超出手工特徵已捕捉的資訊?沒有——CNN 本身顯著比手工特徵
差(p=0.001,兩者用同一組2019–2026共同日期計算,見第一節);增量資訊三項檢查(Spearman、
R²、堆疊)一致指向沒有增量資訊,堆疊AUC顯著低於手工特徵單獨(p=0.033),但判讀為小樣本
驗證年多估一個無資訊量係數帶來的估計雜訊,不是CNN分數主動有害——8折中只有1折堆疊CNN
係數顯著,且逐折正負號不一致。** 任務B兩個N上CNN對最佳攤平基準的AUC差經Holm校正後都
不顯著(p=0.707)。過擬合差距比第9週樹模型溫和,不是過擬合造成落後;
